...
if(detail){
 ...
 <div class="product-grid"><div><div class="product-kicker">PASSION PROJECT / SOFTWARE</div><h1 class="product-title">${p.name.replace(' Font ','<br>Font ')}</h1><p class="product-copy">${p.description}</p><div class="release-meta">${p.platforms.map(x=>`<span>${x}</span>`).join('')}<span>v${p.version}</span><span>${p.status}</span></div><div class="actions"><a class="btn primary" href="#downloads">Choose download</a><a class="btn" href="../contact/">Need help?</a></div></div><div class="product-visual"><div class="guj">અ</div><small>LEGACY → UNICODE</small></div></div>
