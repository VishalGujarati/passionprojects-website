...
if(detail){
 ...
 <section class="section" id="downloads"><div class="section-head"><div><label>DOWNLOADS</label><h2>Choose your<br><em>platform.</em></h2></div><p class="head-copy">Download the latest available package for your platform.</p></div><div class="download-box"><div class="platform"><label>WINDOWS</label><h3>Windows package</h3><p>Latest Windows build.</p><a class="btn primary" href="${p.downloads.windows}">${p.downloads.windows==='#'?'Windows link coming soon':'Download Windows'}</a></div><div class="platform"><label>UBUNTU</label><h3>Ubuntu package</h3><p>Latest Ubuntu/Debian package.</p><a class="btn primary" href="${p.downloads.ubuntu}">${p.downloads.ubuntu==='#'?'Ubuntu link coming soon':'Download Ubuntu'}</a></div></div></section>`}
}
})();
